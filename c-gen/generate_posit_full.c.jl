...
function full_output(op::Symbol, n::Integer, es::Integer)
  ...
  if (op == :div)
    ...
    string(headerfn(op, n, es), """{
      ...
      res->udata = sanitize_div$(n)(lhs->udata, rhs->udata, fres, $(to_p(n, es,:fres)));
  ...
  elseif (op in [:fma, :fms, :nfma, :nfms])
    ...
    string(headerfn(op, n, es), """{
      ...
      res->udata = sanitize_fma$(n)(a->udata, b->udata, c->udata, fres, $(to_p(n, es,:fres)), z_count_valid);
    }""")
  else
    #:fas, :fcp, :fdp, :dpi
    string(headerfn(op, n, es), """{
