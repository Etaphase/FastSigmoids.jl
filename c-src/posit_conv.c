...
extern "C" float p8e0_to_f(p8e0_t pval){
  return p8_to_float(fval.udata, 0, ((uint8_t) 0x00));
}
...
extern "C" float p8e1_to_f(p8e1_t pval){
  return p8_to_float(fval.udata, 1, ((uint8_t) 0x80));
}
...
extern "C" float p8e2_to_f(p8e2_t pval){
  return p8_to_float(fval.udata, 2, ((uint8_t) 0xc0));
}
...
static uint8_t float_to_p8(float fval, int16_t es, int16_t maximum_exponent, int16_t minumum_exponent){
...
static uint8_t float_to_p8_zero_es(float fval){
  ...
  uint32_t frac = ((*ival) << (-24)) & (0x3fffffffL);
...
static float p8_to_float(uint8_t pval, int16_t es, uint8_t es_mask){
  ...
  if (pval.udata == ((uint8_t) 0x80)) return INFINITY;
  if (pval.udata == ((uint8_t) 0x00)) return 0;

  //next, determine the sign of the posit value
  uint8_t pos_val = (pval.sdata < 0) ? -pval.udata : pval.udata;
  ...
  result = (negative ? 0x80000000L : 0x00000000L) |
    (((uint32_t) exponent) << exp_shift) |
    ((((uint32_t) pos_val) << shift) & frac_mask);
...
extern "C" float p16e0_to_f(p16e0_t pval){
  return p16_to_float(fval.udata, 0, ((uint16_t) 0x0000));
}
...
extern "C" float p16e1_to_f(p16e1_t pval){
  return p16_to_float(fval.udata, 1, ((uint16_t) 0x8000));
}
...
extern "C" float p16e2_to_f(p16e2_t pval){
  return p16_to_float(fval.udata, 2, ((uint16_t) 0xc000));
}
...
static uint16_t float_to_p16(float fval, int16_t es, int16_t maximum_exponent, int16_t minumum_exponent){
  //create a result value
  uint16_t res;
...
static float p16_to_float(uint16_t pval, int16_t es, uint16_t es_mask){
  ...
  if (pval.udata == ((uint16_t) 0x8000)) return INFINITY;
  if (pval.udata == ((uint16_t) 0x0000)) return 0;

  //next, determine the sign of the posit value
  uint16_t pos_val = (pval.sdata < 0) ? -pval.udata : pval.udata;
...
extern "C" double p32e0_to_f(p32e0_t pval){
  return p32_to_double(fval.udata, 0, 0x00000000L);
}
...
extern "C" double p32e1_to_f(p32e1_t pval){
  return p32_to_double(fval.udata, 1, 0x80000000L);
}
...
extern "C" double p32e2_to_f(p32e2_t pval){
  return p32_to_double(fval.udata, 2, 0xc0000000L);
}
...
extern "C" double p32e3_to_f(p32e3_t pval){
  return p32_to_double(fval.udata, 3, 0xe0000000L);
}
...
static uint32_t double_to_p32(double fval, int16_t es, int16_t maximum_exponent, int16_t minumum_exponent){
  //create a result value
  uint32_t res;
...
static uint32_t double_to_p32_zero_es(double fval){
  ...
  uint64_t frac = ((*ival) << (-53)) & (0x3fffffffffffffffLL);
...
static double p32_to_double(uint32_t pval, int16_t es, uint32_t es_mask){
  ...
  if (pval.udata == 0x80000000L) return INFINITY;
  if (pval.udata == 0x00000000L) return 0;

  //next, determine the sign of the posit value
  uint32_t pos_val = (pval.sdata < 0) ? -pval.udata : pval.udata;
  ...
  if (inverted){
    ...
    //__builtin_clz has "undefined" state for a value of 0.  W.T.F, C??
    u_regime = (z_posit == 0) ? 64 : __builtin_clz(z_posit);
    s_regime = u_regime - 1;
  }
  //next create the proper exp/frac value by shifting the pos_val, based on the
  //unsigned regime value.
  ...
  result = (negative ? 0x8000000000000000LL : 0x0000000000000000LL) |
    (((uint64_t) exponent) << exp_shift) |
    ((((uint64_t) pos_val) << shift) & frac_mask);
