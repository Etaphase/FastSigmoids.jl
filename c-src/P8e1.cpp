...
P8e1 &P8e1::operator *=(const P8e1 rhs){
  ...
  if (set_nan_jmp()) {
    p8e1_mul_j((p8e1_t *) this, &lhs, &rhs);
  } else {
...
P8e1 &P8e1::operator -=(const P8e1 rhs){
  ...
  if (set_nan_jmp()) {
    p8e1_sub_j((p8e1_t *) this, &lhs, &rhs);
  } else {
    throw domain_error("NaN value obtained in operator -=");
  }
  return (*this);
...
P8e1 &P8e1::operator +=(const P8e1 rhs){
  ...
  if (set_nan_jmp()) {
    p8e1_add_j((p8e1_t *) this, &lhs, &rhs);
  } else {
    throw domain_error("NaN value obtained in operator +=");
  }
  return (*this);
...
P8e1 &P8e1::operator /=(const P8e1 rhs){
  ...
  if (set_nan_jmp()) {
    p8e1_div_j((p8e1_t *) this, &lhs, &rhs);
  } else {
    throw domain_error("NaN value obtained in operator /=");
  }
  return (*this);
